...
test('e2e: load extension, open UI, download video and verify welcome_to_insightai_acadmey file', async () => {
  const extensionPath = path.resolve(
    'C:\\Users\\L0000166\\VS projects\\upwork-test-task-playwright\\release\\loom-downloader'
  ); // should be downloaded from web
  ...
  try {
    ...
    try {
      ...
      await ui.fill('input[name="license"], input[type="text"]', 'test', {
        timeout: 1000,
      });
      const submit = await ui.$(
        'button:has-text("Activate"), button:has-text("Login"), button:has-text("Authorize")'
      );
    ...
    const selectors = ['button:has-text("Download")'];
    ...
    try {
      await page.waitForSelector('#loom-download-manager', {
        timeout: 120_000,
        state: 'visible',
      });
      await ui.screenshot({
        path: `download-manager-appeared.png`,
        fullPage: true,
      });
        ...
        () => {
          const percent =
            document.querySelector(
              '#loom-download-manager [data-role="percent"]'
            )?.textContent ?? '';
          ...
          const fill =
            (
              document.querySelector(
                '#loom-download-manager [data-role="progress-fill"]'
              ) as HTMLElement | null
            )?.style?.width ?? '';
